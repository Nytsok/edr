...
RemComCOMM = "RemCom_communicaton"
    ...
    def doStuff(self, rpctransport):
        ...
        try:
            ...
            if self.__exeFile is None:
                ...
                # The embedded RemCom binary has had its pipe-name IoCs randomized.
                # Mirror those names on the client so the pipes match.
                stdout_name = svc.pipe_stdout
                stdin_name = svc.pipe_stdin
                stderr_name = svc.pipe_stderr
                comm_name = svc.pipe_communicaton
                installService = serviceinstall.ServiceInstall(rpctransport.get_smb_connection(), svc, self.__serviceName, self.__remoteBinaryName)
            else:
                try:
                    ...
                    logging.critical(str(e))
                    sys.exit(1)
                stdout_name = RemComSTDOUT
                stdin_name = RemComSTDIN
                stderr_name = RemComSTDERR
                comm_name = RemComCOMM
            ...
            fid_main = self.openPipe(s,tid,'\\' + comm_name,0x12019f)

            packet = RemComMessage()
            pid = os.getpid()

            packet['Machine'] = ''.join([random.choice(string.ascii_letters) for _ in range(12)])
            ...
            stdin_pipe = RemoteStdInPipe(rpctransport,
                                         r'\%s%s%d' % (stdin_name, packet['Machine'], packet['ProcessID']),
                                         smb.FILE_WRITE_DATA | smb.FILE_APPEND_DATA, installService.getShare())
            stdin_pipe.start()
            stdout_pipe = RemoteStdOutPipe(rpctransport,
                                           r'\%s%s%d' % (stdout_name, packet['Machine'], packet['ProcessID']),
                                           smb.FILE_READ_DATA)
            stdout_pipe.start()
            stderr_pipe = RemoteStdErrPipe(rpctransport,
                                           r'\%s%s%d' % (stderr_name, packet['Machine'], packet['ProcessID']),
                                           smb.FILE_READ_DATA)
            stderr_pipe.start()
